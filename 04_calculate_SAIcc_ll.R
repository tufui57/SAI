...
SAIcc_ll <- function(
  ...
  for(i in 1:nrow(data1)){
    ...
    if(no.neighbourhood == TRUE){
      ...
      dat.x <- analogousCells_in_single_variable(p, data1, a, coordinateNames[1])
    ...
    if(outlierRemoval == TRUE){
      ...
      for(j in climateNames){
        res[[j]] <- check_outliers(p, data1, j, outlierPercent)
      }
    ...
    ###################################################################################### 
    ranges <- lapply(climateNames, get_radius_size, dat = neighbour.window, climateRangeStep = climateRangeStep)
